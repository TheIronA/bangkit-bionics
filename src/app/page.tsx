      ...
      <section id="solution" style={{ padding: "7rem clamp(1.25rem, 6vw, 5rem)", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "3rem", alignItems: "stretch" }}>
          ...
          <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
            {[
              { name: "FMG + EMG Sensor Fusion", desc: "We capture two complementary signals from the residual limb: EMG (electrical muscle activity via surface electrodes) and FMG (force myography — pressure changes as muscles contract). Peer-reviewed research shows combined FMG+EMG classification achieves >97% accuracy even under impeded-contact conditions (sweat, clothing isolation) versus ~84–91% for EMG alone under the same conditions — a meaningful gap for a device worn in tropical Southeast Asia (Ke et al., Sensors, 2020). Both sensors are non-invasive, wearable, and low-cost. This is a hardware and signal-processing design decision, not a research moonshot." },
              { name: "On-Device ML Grip Classification", desc: "A patient-specific neural network classifier — trained on the user's own FMG+EMG data — maps 4–6 grip intentions directly on the microcontroller. No cloud dependency, no latency. Transfer learning reduces per-patient calibration time and adapts the model as the residual limb changes over time. Stage one uses threshold-based control; the neural classifier is the next build stage. This is applied engineering on a validated signal pipeline, not novel neuroscience." },
        ...
        <div style={{ marginTop: "5rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "1rem" }}>
            {[
              { n: "01", title: "FMG + EMG Sensor Fusion", desc: "Two complementary signals captured from the residual limb: EMG (electrical muscle activity via surface electrodes) and FMG (force myography — pressure as muscles contract). Peer-reviewed research shows FMG+EMG fusion achieves >97% classification accuracy under impeded-contact conditions (sweat, clothing isolation) versus ~84–91% for EMG alone — directly relevant to tropical-climate wearability (Ke et al., Sensors, 2020). Both sensors are non-invasive; no surgery, no skin penetration." },
              { n: "02", title: "On-Device Neural Net Classification", desc: "A compact neural network — trained on the patient's own FMG+EMG data — classifies 4–6 grip intentions directly on the microcontroller. No cloud, no latency. Transfer learning adapts the model to new users and accommodates residual limb changes over time, reducing calibration overhead at each fitting. Stage one is threshold-based; the neural classifier is the next build stage." },
