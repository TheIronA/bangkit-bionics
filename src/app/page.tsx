      ...
      <section style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", textAlign: "center", padding: "8rem clamp(1.25rem, 6vw, 4rem) 5rem", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 800 }}>
          ...
          <div ref={heroRef} className="fade-up" style={{ display: "flex", gap: "clamp(1.5rem, 5vw, 4rem)", justifyContent: "center", flexWrap: "wrap", paddingTop: "2rem", borderTop: "1px solid rgba(255,255,255,0.08)" }}>
            ...
            <div style={{ textAlign: "center" }}>
              <div style={{ fontFamily: "var(--font-heading)", fontSize: "clamp(1.8rem, 3vw, 2.5rem)", fontWeight: 700, color: "var(--teal)", lineHeight: 1.1 }}>SGD 15K–50K</div>
              <div style={{ fontSize: "0.85rem", color: "var(--white-60)", marginTop: "0.35rem" }}>Current market range for myoelectric arms in Singapore. We enter at SGD 3,000.</div>
            </div>
          </div>
        </div>
      </section>
      ...
      <section id="why-now" style={{ padding: "7rem clamp(1.25rem, 6vw, 5rem)", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <h2 className="fade-up" style={{ fontFamily: "var(--font-heading)", fontWeight: 700, fontSize: "clamp(1.8rem, 3.5vw, 2.8rem)", letterSpacing: "-0.02em", marginBottom: "3rem" }}>
            Five forces have aligned.<br />The window is open.
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: "1.5rem" }}>
            {[
              ...
              { n: "03", title: "EMG sensors commoditised", desc: "Surface EMG sensors that once cost USD 2,000 per channel now retail under USD 20. Microcontrollers with sufficient processing power for pattern recognition cost under USD 5. The electronics BOM for a myoelectric prosthetic is now under RM 400." },
              { n: "04", title: "Defined regulatory pathway", desc: "Malaysia has an established medical-device approval framework, giving the team a concrete route to plan around while final device classification is confirmed with MDA." },
              { n: "05", title: "Regional scale is undeniable", desc: "Southeast Asia is home to roughly 695 million people, yet access to advanced assistive technology remains deeply uneven." },
            ].map((item) => (
      ...
      <section id="solution" style={{ padding: "7rem clamp(1.25rem, 6vw, 5rem)", maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "3rem", alignItems: "stretch" }}>
          ...
          <div className="fade-up" style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
            {[
              { name: "FMG + EMG Sensor Fusion", desc: "We capture two complementary signals from the residual limb: EMG (electrical muscle activity via surface electrodes) and FMG (force myography — pressure changes as muscles contract). Ke et al. (2020) demonstrated that FMG+EMG fusion can achieve >97% accuracy even under impeded-contact conditions (sweat, clothing isolation) versus ~84–91% for EMG alone under the same conditions — a meaningful gap for a device worn in tropical Southeast Asia (Sensors, MDPI; doi:10.3390/s20174775). We target comparable performance using the same sensor modality; to be validated in our clinical trial. Both sensors are non-invasive, wearable, and low-cost. This is a hardware and signal-processing design decision, not a research moonshot." },
              { name: "On-Device ML Grip Classification", desc: "A patient-specific neural network classifier — trained on the user's own FMG+EMG data — maps 3 core grip intentions (power grasp, pinch, open) directly on the microcontroller, with 4–6 grips as a post-prototype target. No cloud dependency, no latency. Transfer learning reduces per-patient calibration time and adapts the model as the residual limb changes over time. Stage one uses threshold-based control; the neural classifier is the next build stage. This is applied engineering on a validated signal pipeline, not novel neuroscience." },
        ...
        <div style={{ marginTop: "5rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "1rem" }}>
            {[
              { n: "01", title: "FMG + EMG Sensor Fusion", desc: "Two complementary signals captured from the residual limb: EMG (electrical muscle activity via surface electrodes) and FMG (force myography — pressure as muscles contract). Ke et al. (2020) demonstrated that FMG+EMG fusion can achieve >97% classification accuracy under impeded-contact conditions (sweat, clothing isolation) versus ~84–91% for EMG alone — directly relevant to tropical-climate wearability (Sensors, MDPI; doi:10.3390/s20174775). We target comparable performance; to be validated in our prototype trials. Both sensors are non-invasive; no surgery, no skin penetration." },
              { n: "02", title: "On-Device Neural Net Classification", desc: "A compact neural network — trained on the patient's own FMG+EMG data — classifies 3 core grip intentions (power grasp, pinch, open) directly on the microcontroller, with 4–6 as a post-prototype target. No cloud, no latency. Transfer learning adapts the model to new users and accommodates residual limb changes over time, reducing calibration overhead at each fitting. Stage one is threshold-based; the neural classifier is the next build stage." },
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "1.5rem", marginBottom: "3.5rem" }}>
            {[
              ...
              { value: "USD 80M", label: "Beachhead TAM", sub: "Addressable market estimate, Malaysia + Singapore (not a revenue target)" },
            ].map((m) => (
          ...
          <p className="fade-up" style={{ fontSize: "0.75rem", color: "rgba(255,255,255,0.45)", marginBottom: "2rem", letterSpacing: "0.02em" }}>
            Sources: UN World Population Prospects 2024 Revision; Grand View Research Prosthetics and Orthotics Market Summary (updated 2024). Beachhead TAM is an internal estimate. FMG+EMG accuracy stat: Ke et al., "An Ultra-Sensitive Modular Hybrid EMG–FMG Sensor with Floating Electrodes", Sensors (MDPI), 2020, doi:10.3390/s20174775.
          </p>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div className="fade-up" style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem", minWidth: 640 }}>
              ...
              <tbody>
                {[
                  { company: "BANGKIT Bionics", highlight: true, price: "RM 4.5K\u201314.5K", control: "FMG+EMG fusion + ML classifier", thumb: "Motorized (prototype target)", socket: "Custom 3D-printed socket", model: "KL build, SG/UK sales", reg: "NTIS + MDA → CE Mark" },
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: "1.5rem" }}>
            {[
              ...
              { title: "FMG+EMG fusion + ML at this price point", desc: "The primary technical moat: fused FMG+EMG sensors feeding a patient-specific neural net classifier, running on a RM 45 microcontroller, at a device price of RM 9,500. Ke et al. (2020) demonstrated that FMG+EMG fusion can achieve >97% accuracy under impeded-contact conditions (sweat, clothing isolation) versus ~84–91% for EMG alone — directly relevant to tropical-climate wearability (Sensors, MDPI, 2020). Transfer learning adapts the model per patient and accommodates limb changes over time. No competitor in this price bracket ships this sensor+ML combination — only binary threshold control or manual repositioning. To be validated in our clinical trial." },
              ...
              { title: "Data and talent compounding", desc: "Every patient fit and retraining session adds to a proprietary multi-modal FMG+EMG dataset that competitors cannot replicate quickly. Transfer-learning models improve as the dataset grows. Combined with Malaysia's underused biomedical engineering talent base, that compounds into a harder-to-copy company over time." },
            ].map((adv) => (
